"""
福建省 2025 年 SPI_30d 干旱等级标签脚本

脚本功能：
1. 读取 2025 年 SPI_30d NetCDF 结果。
...
# ============================================================
# 输入文件：第一步 SPI 计算得到的 2025 年逐日 SPI_30d 结果。
SPI_FILE = Path(r"E:\forest_microclimate\ForestMicroclimate\results\daily_SPI_result\Fujian_daily_SPI30d_2025.nc")

# 输出目录：所有图、表、标签 NC 都写入该目录。
OUTPUT_DIR = Path(r"E:\forest_microclimate\ForestMicroclimate\results\daily_SPI_mark")

# 输出 NC 文件名按要求使用英文小写，其他图表文件使用中文命名。
OUTPUT_NC = OUTPUT_DIR / "fujian_drought_label_2025_daily.nc"
OUTPUT_CSV = OUTPUT_DIR / "福建省2025年SPI_30d干旱等级逐日统计表.csv"
OUTPUT_SITE_CSV = OUTPUT_DIR / "福建省2025年SPI_30d站点干旱等级与SPI值表（daily）.csv"
OUTPUT_LINE_CHART = OUTPUT_DIR / "福建省2025年SPI_30d干旱等级逐日占比堆叠面积图.png"
...
# 1. 数据参数：变量名、年份、坐标名
# ============================================================
# SPI_VAR：NetCDF 中待标记的 SPI 变量名。当前文件中变量名为 SPI_30d。
SPI_VAR = "SPI_30d"
...
def build_site_monthly_table(spi_2025: xr.DataArray, ds_label: xr.Dataset, site_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, site in make_progress(
        ...
        for month_index, month_time in enumerate(month_times):
            ...
            row[f"{month_prefix}_SPI_30d"] = round(spi_value, 4) if np.isfinite(spi_value) else np.nan
...
def draw_monthly_maps(ds_label: xr.Dataset, site_df: pd.DataFrame) -> None:
    """逐月绘制 SPI_30d 干旱等级空间分布图，并叠加福建省行政边界和站点位置。"""
    ...
    for month_index in make_progress(
        ...
        fig.tight_layout()
        fig_path = OUTPUT_DIR / f"SPI_30d_{month_time.strftime('%Y_%m_%d')}.png"
...
def draw_monthly_line_chart(df_stat: pd.DataFrame) -> None:
    ...
    ax.set_title(
        "Daily SPI_30d Drought Level Percentage in Fujian, 2025",
...
def main() -> None:
    ...
    try:
        ...
        print(f"4. 逐日空间分布图：{OUTPUT_DIR}\\SPI_30d_YYYY_MM_DD.png")
