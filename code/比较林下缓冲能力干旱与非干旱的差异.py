...
# 是否直接读取已经对齐好的逐小时温度表。
# True：跳过 TOMST 15min 聚合、ERA5 8760张TIF提取和逐小时合并，直接进入β计算与绘图。
# False：重新从 TOMST_DIR 和 ERA5_TIF_DIR 读取原始数据并生成逐小时对齐表。
USE_HOURLY_ALIGNED_CACHE = True

# 已对齐逐小时温度表路径。
# 表格必须是一行一个“站点 × UTC小时”，并且该行的15cm观测温度与ERA5 2m温度已经时间对齐。
HOURLY_ALIGNED_CACHE_CSV = (
    r"E:\forest_microclimate\ForestMicroclimate\results"
    r"\时间序列图\逐小时温度对齐表.csv"
)

# 缓存表列名映射：
#   Site_ID          ：站点编号
#   Longitude        ：站点经度（可选保留，用于完整性检查和导出追溯）
#   Latitude         ：站点纬度（可选保留，用于完整性检查和导出追溯）
#   Time_UTC         ：UTC小时时间
#   Observed_T15cm_C ：15cm观测温度，已由15min聚合为1h
#   ERA5_T2m_C       ：ERA5 2m温度，已按站点经纬度从TIF提取，单位为℃
HOURLY_CACHE_SITE_COL = "Site_ID"
HOURLY_CACHE_LON_COL = "Longitude"
HOURLY_CACHE_LAT_COL = "Latitude"
HOURLY_CACHE_TIME_COL = "Time_UTC"
HOURLY_CACHE_MICRO_COL = "Observed_T15cm_C"
HOURLY_CACHE_MACRO_COL = "ERA5_T2m_C"
...
def load_hourly_aligned_cache(path):
    """
    读取已经完成“站点 × UTC小时”对齐的温度缓存表，直接作为后续β计算主表。

    使用场景：
      - TOMST 15min 数据已经预先聚合成逐小时 Observed_T15cm_C；
      - ERA5 2m 温度已经按站点经纬度从逐小时TIF提取为 ERA5_T2m_C；
      - 两者已经按 Site_ID + Time_UTC 对齐到同一行。

    这样可以跳过最耗时的两步：
      1. 反复读取27个TOMST CSV并重采样；
      2. 反复遍历8760张ERA5 TIF并按站点坐标提取。

    返回：
      merged : DataFrame，列至少包含 site_id / datetime / T_micro / T_macro，
               可直接传入 compute_paired_betas() 和 analyze_and_plot()。
    """
    print("=" * 65)
    print("Step 1-3: 读取逐小时温度对齐缓存表（跳过TOMST和ERA5原始提取）")
    print(f"  缓存表：{path}")

    if not os.path.exists(path):
        raise FileNotFoundError(
            f"逐小时温度对齐缓存表不存在：{path}\n"
            f"请检查 HOURLY_ALIGNED_CACHE_CSV，或将 USE_HOURLY_ALIGNED_CACHE 改为 False 重新提取。"
        )

    cache = pd.read_csv(path)
    required = [
        HOURLY_CACHE_SITE_COL,
        HOURLY_CACHE_TIME_COL,
        HOURLY_CACHE_MICRO_COL,
        HOURLY_CACHE_MACRO_COL,
    ]
    validate_required_columns(cache, required, "逐小时温度对齐缓存表")

    keep_cols = required.copy()
    optional_cols = [HOURLY_CACHE_LON_COL, HOURLY_CACHE_LAT_COL]
    keep_cols.extend([c for c in optional_cols if c in cache.columns])
    cache = cache[keep_cols].copy()

    rename_map = {
        HOURLY_CACHE_SITE_COL: 'site_id',
        HOURLY_CACHE_TIME_COL: 'datetime',
        HOURLY_CACHE_MICRO_COL: 'T_micro',
        HOURLY_CACHE_MACRO_COL: 'T_macro',
    }
    if HOURLY_CACHE_LON_COL in cache.columns:
        rename_map[HOURLY_CACHE_LON_COL] = 'lon'
    if HOURLY_CACHE_LAT_COL in cache.columns:
        rename_map[HOURLY_CACHE_LAT_COL] = 'lat'
    cache = cache.rename(columns=rename_map)

    cache['site_id'] = cache['site_id'].apply(normalize_site_id)
    cache['datetime'] = pd.to_datetime(cache['datetime'], errors='coerce').dt.tz_localize(None)
    cache['T_micro'] = pd.to_numeric(cache['T_micro'], errors='coerce')
    cache['T_macro'] = pd.to_numeric(cache['T_macro'], errors='coerce')
    if 'lon' in cache.columns:
        cache['lon'] = pd.to_numeric(cache['lon'], errors='coerce')
    if 'lat' in cache.columns:
        cache['lat'] = pd.to_numeric(cache['lat'], errors='coerce')

    before = len(cache)
    cache = cache.dropna(subset=['site_id', 'datetime', 'T_micro', 'T_macro']).copy()
    removed = before - len(cache)
    if removed > 0:
        print(f"  [提示] 删除 {removed:,} 行缺少站点、时间或温度的记录")

    duplicate_count = cache.duplicated(subset=['site_id', 'datetime']).sum()
    if duplicate_count > 0:
        print(f"  [提示] 发现 {duplicate_count:,} 行重复 site_id + datetime，保留最后一条")
        cache = (
            cache.sort_values(['site_id', 'datetime'])
            .drop_duplicates(subset=['site_id', 'datetime'], keep='last')
        )

    # 后续分析函数统一只使用这四列；经纬度会在导出时再从 sites 表补回。
    # 这样可避免缓存表自带 lon/lat 与 export_hourly_temperature_tables() 内部合并产生列名冲突。
    merged = (
        cache[['site_id', 'datetime', 'T_micro', 'T_macro']]
        .sort_values(['site_id', 'datetime'])
        .reset_index(drop=True)
    )

    print(f"  ✓ 缓存表读取完成：{merged['site_id'].nunique()} 个站点，{len(merged):,} 条逐小时记录")
    print(f"  时间范围：{merged['datetime'].min()} → {merged['datetime'].max()}（按UTC解释）")
    print(f"  T_micro范围：{merged['T_micro'].min():.2f} ~ {merged['T_micro'].max():.2f} ℃")
    print(f"  T_macro范围：{merged['T_macro'].min():.2f} ~ {merged['T_macro'].max():.2f} ℃")
    return merged


def export_hourly_temperature_tables(micro, macro, merged, sites_df, output_dir):
    """
    导出逐小时温度明细表，便于独立检查ERA5提取值和TOMST小时聚合值。

    输出两个CSV：
      1. ERA5逐小时提取表.csv
         每行 = 一个站点 × 一个小时；包含站点经纬度和ERA5 2m温度。
      2. 逐小时温度对齐表.csv
         每行 = 一个站点 × 一个小时；包含站点经纬度、TOMST 15cm小时均值、
         ERA5 2m温度。该表与后续OLS分析使用的逐小时数据一致。

    时间说明：
      datetime 为UTC时间，与TOMST data_time和ERA5 TIF时间保持一致。
    """
    ensure_dir(output_dir)
    site_lookup = sites_df[['site_id', 'lon', 'lat']].copy()
    ...
    era5_export = macro.merge(site_lookup, on='site_id', how='left')
    era5_export = add_time_columns(era5_export)
    era5_export = era5_export[
        ['site_id', 'lon', 'lat', 'datetime', 'date', 'year', 'month', 'day', 'hour', 'T_macro']
    ].sort_values(['site_id', 'datetime'])
    era5_path = os.path.join(output_dir, "ERA5逐小时提取表.csv")
    era5_export.to_csv(era5_path, index=False, encoding='utf-8-sig')
    print(f"  ERA5逐小时提取表 → {era5_path}")

    aligned_export = merged.merge(site_lookup, on='site_id', how='left')
    aligned_export = add_time_columns(aligned_export)
    aligned_export = aligned_export[
        ['site_id', 'lon', 'lat', 'datetime', 'date', 'year', 'month', 'day', 'hour', 'T_micro', 'T_macro']
    ].sort_values(['site_id', 'datetime'])
    aligned_path = os.path.join(output_dir, "逐小时温度对齐表.csv")
    aligned_export.to_csv(aligned_path, index=False, encoding='utf-8-sig')
    print(f"  逐小时温度对齐表 → {aligned_path}")


def compute_spi_reference_site_summary(beta_table, merged, daily_spi_df,
                                       day_mask_func, beta_col, r2_col,
                                       n_col, day_count_col):
    """
    计算“全部极端干旱事件 vs 指定SPI参考期”的站点级汇总表。

    通用规则：
      1. 只纳入有目标极端干旱事件的站点；
      2. 干旱期β：每个站点所有极端干旱事件的 beta_drought 取中位数；
      3. 参考期β：按 day_mask_func 从逐日SPI表筛日期，再把这些日期对应
         的逐小时 T_micro / T_macro 数据统一做一次OLS；
      4. 不再按短事件/长事件分流。

    参数：
      day_mask_func : 函数，输入单站点逐日SPI表，返回布尔Series。
                      例如：
                        lambda df: df['daily_spi'] > CONTROL_SPI_THRESHOLD
                        lambda df: (df['daily_spi'] > -0.5) & (df['daily_spi'] < 0.5)
      beta_col/r2_col/n_col/day_count_col : 输出列名，便于同时生成多套参考期汇总表。

    返回列：
      site_id, beta_drought, 指定参考期β/R²/n/天数, event_count, long_event_count
    """
    if beta_table.empty or merged is None or daily_spi_df is None:
        return pd.DataFrame()

    drought_summary = (
        beta_table.groupby('site_id')
        .agg(
            beta_drought=('beta_drought', 'median'),
            event_count=('event_id', 'count'),
            long_event_count=('is_long_drought', 'sum'),
        )
        .reset_index()
    )
    ...
    for _, row in drought_summary.iterrows():
        ...
        if site_daily_spi.empty:
            day_mask = pd.Series(False, index=site_daily_spi.index)
        else:
            day_mask = day_mask_func(site_daily_spi).fillna(False)
        ...
        reference_hours = site_data[site_data.index.normalize().isin(reference_days)]
        beta_reference = run_ols(
            reference_hours.index,
            reference_hours['T_micro'],
            reference_hours['T_macro']
        )

        records.append({
            'site_id': site_id,
            'beta_drought': row['beta_drought'],
            beta_col: beta_reference['beta'],
            r2_col: beta_reference['r2'],
            n_col: beta_reference['n'],
            day_count_col: len(reference_days),
            'event_count': row['event_count'],
            'long_event_count': row['long_event_count'],
        })

    return pd.DataFrame(records)


def compute_spi_nondrought_site_summary(beta_table, merged, daily_spi_df):
    """
    计算“全部极端干旱事件 vs SPI非干旱期”的站点级汇总表。

    非干旱期定义：Daily_SPI_30d > CONTROL_SPI_THRESHOLD（当前为 -0.5）。
    """
    return compute_spi_reference_site_summary(
        beta_table, merged, daily_spi_df,
        day_mask_func=lambda df: df['daily_spi'] > CONTROL_SPI_THRESHOLD,
        beta_col='beta_spi_nondrought',
        r2_col='r2_spi_nondrought',
        n_col='n_spi_nondrought',
        day_count_col='spi_nondrought_days',
    )


def compute_spi_normal_site_summary(beta_table, merged, daily_spi_df):
    """
    计算“全部极端干旱事件 vs 正常SPI期”的站点级汇总表。

    正常SPI期定义：-0.5 < Daily_SPI_30d < 0.5。
    这组图用于排除偏湿期，只把接近正常水分状态的日期作为参考期。
    """
    return compute_spi_reference_site_summary(
        beta_table, merged, daily_spi_df,
        day_mask_func=lambda df: (
            (df['daily_spi'] > CONTROL_SPI_THRESHOLD) &
            (df['daily_spi'] < 0.5)
        ),
        beta_col='beta_spi_normal',
        r2_col='r2_spi_normal',
        n_col='n_spi_normal',
        day_count_col='spi_normal_days',
    )
...
def analyze_and_plot(beta_table, control_quality, site_coverage, output_dir,
    ...
    if not site_summary.empty:
        ...
        two_period_summary_path = os.path.join(output_dir, "站点级_短事件干旱与非干旱β汇总表.csv")
        two_period_summary.to_csv(two_period_summary_path, index=False, encoding='utf-8-sig')
        print(f"  站点级短事件干旱与非干旱汇总表 → {two_period_summary_path}")

    # ── 5b-2. 总体Drought vs Non-drought汇总：长短事件合并，不再分流 ──
    # 这张表专门服务“总体_干旱与非干旱β箱线图/差值图”：
    #   - 只纳入有目标极端干旱事件的站点；
    #   - Drought β = 该站点所有目标极端干旱事件 beta_drought 的中位数；
    #   - Non-drought β = 该站点全年 Daily_SPI_30d > CONTROL_SPI_THRESHOLD
    #     的日期对应逐小时温度，统一重新做一次 OLS；
    #   - 不按短事件/长事件分流，因此总体图样本量不再只受短事件限制。
    spi_nondrought_summary = compute_spi_nondrought_site_summary(
        beta_table, merged, daily_spi_df
    )
    if spi_nondrought_summary.empty:
        print("  [提示] 无可用的SPI非干旱期站点汇总，跳过总体干旱与非干旱图及检验。")
    else:
        spi_nondrought_summary_path = os.path.join(output_dir, "站点级_干旱与非干旱β汇总表.csv")
        spi_nondrought_summary.to_csv(
            spi_nondrought_summary_path, index=False, encoding='utf-8-sig'
        )
        print(f"  站点级干旱与SPI非干旱汇总表 → {spi_nondrought_summary_path}")

    # ── 5b-3. 新增总体参考期：正常SPI期（-0.5 < SPI < 0.5）──
    # 目的：
    #   在保留原有“SPI > -0.5 非干旱期”的同时，额外输出一组更严格的参考期图。
    #   该规则排除了偏湿期（SPI >= 0.5），使参考期更接近“正常水分状态”。
    spi_normal_summary = compute_spi_normal_site_summary(
        beta_table, merged, daily_spi_df
    )
    if spi_normal_summary.empty:
        print("  [提示] 无可用的正常SPI期站点汇总，跳过总体干旱与正常SPI期图及检验。")
    else:
        spi_normal_summary_path = os.path.join(output_dir, "站点级_干旱与正常SPI期β汇总表.csv")
        spi_normal_summary.to_csv(
            spi_normal_summary_path, index=False, encoding='utf-8-sig'
        )
        print(f"  站点级干旱与正常SPI期汇总表 → {spi_normal_summary_path}")
    ...
    if not spi_nondrought_summary.empty:
        valid_spi_nondrought = spi_nondrought_summary.dropna(
            subset=['beta_drought', 'beta_spi_nondrought']
        )
        report_nondrought = do_wilcoxon(
            valid_spi_nondrought['beta_drought'],
            valid_spi_nondrought['beta_spi_nondrought'],
            f"全部极端事件：干旱期 vs SPI非干旱期（Daily_SPI_30d > {CONTROL_SPI_THRESHOLD}）"
        )
    else:
        valid_spi_nondrought = pd.DataFrame()
        report_nondrought = "全部极端事件：无可用SPI非干旱期站点汇总，跳过总体干旱与非干旱检验。"

    if not spi_normal_summary.empty:
        valid_spi_normal = spi_normal_summary.dropna(
            subset=['beta_drought', 'beta_spi_normal']
        )
        report_normal_spi = do_wilcoxon(
            valid_spi_normal['beta_drought'],
            valid_spi_normal['beta_spi_normal'],
            f"全部极端事件：干旱期 vs 正常SPI期（{CONTROL_SPI_THRESHOLD} < Daily_SPI_30d < 0.5）"
        )
    else:
        valid_spi_normal = pd.DataFrame()
        report_normal_spi = "全部极端事件：无可用正常SPI期站点汇总，跳过总体干旱与正常SPI期检验。"
    ...
    report_lines = [
        ...
        f"  总体干旱-非干旱图：长短事件合并；非干旱期定义为 Daily_SPI_30d > {CONTROL_SPI_THRESHOLD} 的日期",
        f"  总体干旱-正常SPI期图：长短事件合并；正常SPI期定义为 {CONTROL_SPI_THRESHOLD} < Daily_SPI_30d < 0.5 的日期",
        "  β计算方法  ：OLS回归 T_micro = α + β × T_macro",
        "    参考文献 ：Ma et al. (2025) Agricultural and Forest Meteorology",
        "  站点聚合   ：每站点取所有同类事件统计量的中位数（独立站点为检验单位）",
        "  统计检验   ：短事件前/后对照用配对Wilcoxon；总体干旱-非干旱用长短事件合并站点级配对Wilcoxon；长事件Δβ用单样本Wilcoxon against 0",
        "    参考文献 ：Hollander & Wolfe (1999) Nonparametric Statistical Methods",
    ...
    # ── 5f. Overall boxplot: Non-drought vs Drought（长短事件合并） ──
    # 总体图规则：
    #   - Drought：每个站点所有目标极端干旱事件 beta_drought 的中位数；
    #   - Non-drought：该站点所有 Daily_SPI_30d > CONTROL_SPI_THRESHOLD
    #     的日期，对应逐小时温度重新做一次OLS；
    #   - 不再只使用短事件的等长对照期，避免总体样本量被短事件数量限制。
    if not spi_nondrought_summary.empty:
        order_two = ['Non-drought', 'Drought']
        nondrought_vals = (
            spi_nondrought_summary[['site_id', 'beta_spi_nondrought']]
            .dropna()
            .rename(columns={'beta_spi_nondrought': 'beta'})
        )
        nondrought_vals['Period'] = order_two[0]
        drought_vals_two = (
            spi_nondrought_summary[['site_id', 'beta_drought']]
            .dropna()
            .rename(columns={'beta_drought': 'beta'})
        )
        ...
        if not plot_long_two.empty:
            ...
            for i, col in enumerate(['beta_spi_nondrought', 'beta_drought']):
                n_valid = spi_nondrought_summary[col].notna().sum()
            ...
            ax.set_title(
                "Overall Comparison of Climate Buffering Index\n"
                f"Non-drought (Daily_SPI_30d > {CONTROL_SPI_THRESHOLD}) vs Drought",
                fontsize=TITLE_FONTSIZE, pad=12
            )
            plt.tight_layout()
    ...
    # ── 5g. Overall delta-beta plot: Non-drought vs Drought（站点级配对） ──
    delta_nondrought = spi_nondrought_summary[
        ['site_id', 'beta_drought', 'beta_spi_nondrought']
    ].dropna().copy()
    if not delta_nondrought.empty:
        delta_nondrought['Comparison'] = 'Drought - Non-drought'
        delta_nondrought['delta_beta'] = (
            delta_nondrought['beta_drought'] - delta_nondrought['beta_spi_nondrought']
        )
        fig, ax = plt.subplots(figsize=FIG_SIZE)
        sns.boxplot(
        ...
        ax.set_title(
            f"Overall Delta-β: Drought vs Non-drought\n"
            f"Non-drought days: Daily_SPI_30d > {CONTROL_SPI_THRESHOLD}",
            fontsize=TITLE_FONTSIZE, pad=12
        )
        plt.tight_layout()
    ...
    # ── 5h. New overall plot: Normal-SPI period vs Drought（-0.5 < SPI < 0.5）──
    # 这组图不替换原来的“SPI > -0.5 非干旱期”图，而是新增一组更严格的参考期：
    #   - Drought：每个站点所有目标极端干旱事件 beta_drought 的中位数；
    #   - Normal-SPI：该站点所有 CONTROL_SPI_THRESHOLD < Daily_SPI_30d < 0.5
    #     的日期，对应逐小时温度重新做一次OLS；
    #   - 用于检查“排除偏湿期后”的干旱-正常期差异是否仍然存在。
    if not spi_normal_summary.empty:
        order_normal = ['Normal-SPI', 'Drought']
        normal_vals = (
            spi_normal_summary[['site_id', 'beta_spi_normal']]
            .dropna()
            .rename(columns={'beta_spi_normal': 'beta'})
        )
        normal_vals['Period'] = order_normal[0]
        drought_vals_normal = (
            spi_normal_summary[['site_id', 'beta_drought']]
            .dropna()
            .rename(columns={'beta_drought': 'beta'})
        )
        drought_vals_normal['Period'] = order_normal[1]
        plot_long_normal = pd.concat([normal_vals, drought_vals_normal], ignore_index=True)

        if not plot_long_normal.empty:
            fig, ax = plt.subplots(figsize=FIG_SIZE)
            sns.boxplot(
                data=plot_long_normal, x='Period', y='beta', order=order_normal,
                palette=BOX_COLORS_TWO_PERIOD, width=BOX_WIDTH, linewidth=BOX_LINEWIDTH,
                flierprops=dict(marker='o', markerfacecolor='gray', markersize=3, alpha=0.5),
                ax=ax
            )
            ...
            y_max_normal = plot_long_normal['beta'].dropna().max()
            beta_range_normal = (
                plot_long_normal['beta'].dropna().max() -
                plot_long_normal['beta'].dropna().min()
            )
            offset_normal = beta_range_normal * 0.06 if beta_range_normal > 0 else 0.05
            if len(valid_spi_normal) >= 5:
                _, p_normal = wilcoxon(
                    valid_spi_normal['beta_drought'],
                    valid_spi_normal['beta_spi_normal'],
                    alternative='two-sided'
                )
                y_line = y_max_normal + offset_normal
                ax.plot([0, 1], [y_line, y_line], color=SIGNIFICANCE_LINE_COLOR, linewidth=SIGNIFICANCE_LINEWIDTH)
                sig_str = f"p={p_normal:.3f}" + (" *" if p_normal < 0.05 else " ns")
                ax.text(0.5, y_line + offset_normal * 0.3, sig_str, ha='center', va='bottom', fontsize=9)
            ax.axhline(REFERENCE_LINE_Y, color=REFERENCE_LINE_COLOR, linestyle=REFERENCE_LINE_STYLE,
                       alpha=REFERENCE_LINE_ALPHA, linewidth=1)
            ax.text(1.48, 1.002, 'β=1\n(No buffering)', fontsize=8, color='gray', va='bottom', ha='right')
            for i, col in enumerate(['beta_spi_normal', 'beta_drought']):
                n_valid = spi_normal_summary[col].notna().sum()
                ax.text(i, plot_long_normal['beta'].dropna().min() - offset_normal * 0.8,
                        f"n={n_valid}", ha='center', fontsize=9, color=SAMPLE_N_COLOR)
            ax.set_xlabel("")
            ax.set_ylabel("Climate Buffering Index (β)", fontsize=AXIS_LABEL_FONTSIZE)
            ax.set_title(
                "Overall Comparison of Climate Buffering Index\n"
                f"Normal-SPI ({CONTROL_SPI_THRESHOLD} < Daily_SPI_30d < 0.5) vs Drought",
                fontsize=TITLE_FONTSIZE, pad=12
            )
            plt.tight_layout()
            box_path_normal = os.path.join(overall_plot_dir, "总体_干旱与正常SPI期β箱线图.png")
            plt.savefig(box_path_normal, dpi=FIG_DPI, bbox_inches='tight')
            plt.close()
            print(f"  总体干旱与正常SPI期箱线图 → {box_path_normal}")

    delta_normal = spi_normal_summary[
        ['site_id', 'beta_drought', 'beta_spi_normal']
    ].dropna().copy()
    if not delta_normal.empty:
        delta_normal['Comparison'] = 'Drought - Normal-SPI'
        delta_normal['delta_beta'] = (
            delta_normal['beta_drought'] - delta_normal['beta_spi_normal']
        )
        fig, ax = plt.subplots(figsize=FIG_SIZE)
        sns.boxplot(
            data=delta_normal, x='Comparison', y='delta_beta',
            palette=['#FF8A65'], width=BOX_WIDTH, linewidth=BOX_LINEWIDTH, ax=ax
        )
        sns.stripplot(
            data=delta_normal, x='Comparison', y='delta_beta',
        ...
        ax.axhline(0.0, color='gray', linestyle='--', alpha=0.5, linewidth=1)
        ax.set_xlabel("")
        ax.set_ylabel("Δβ (Drought - Normal-SPI)", fontsize=AXIS_LABEL_FONTSIZE)
        ax.set_title(
            f"Overall Delta-β: Drought vs Normal-SPI\n"
            f"Normal-SPI days: {CONTROL_SPI_THRESHOLD} < Daily_SPI_30d < 0.5",
            fontsize=TITLE_FONTSIZE, pad=12
        )
        plt.tight_layout()
        delta_path_normal = os.path.join(overall_plot_dir, "总体_干旱与正常SPI期β差值图.png")
        plt.savefig(delta_path_normal, dpi=FIG_DPI, bbox_inches='tight')
        plt.close()
        print(f"  总体干旱与正常SPI期差值图 → {delta_path_normal}")

    # ── 5i. Overall plot: long-drought Δβ (%) relative deviation ──
    ...
    # ── 5j. Site-level plots: paired line plot and event bar plot（3-period, short events only） ──
...
def main():
    ...
    执行顺序：
        若 USE_HOURLY_ALIGNED_CACHE=True：
            Step 1-3 → 直接读取逐小时温度对齐缓存表（推荐，速度最快）
        若 USE_HOURLY_ALIGNED_CACHE=False：
    ...
    【调试建议】
        常规分析建议保持 USE_HOURLY_ALIGNED_CACHE=True。
        只有当TOMST原始CSV、ERA5 TIF或站点坐标发生变化，需要重新生成逐小时表时，
        才把 USE_HOURLY_ALIGNED_CACHE 改为 False。
    """
    try:
        print("=" * 65)
        ...
        # ── 执行各步骤 ──
        if USE_HOURLY_ALIGNED_CACHE:
            merged = load_hourly_aligned_cache(HOURLY_ALIGNED_CACHE_CSV)
            # 为了继续生成标准化输出表，从缓存主表派生 micro/macro。
            # 这里不会重新读取TOMST，也不会重新遍历ERA5 TIF。
            micro = merged[['site_id', 'datetime', 'T_micro']].copy()
            macro = merged[['site_id', 'datetime', 'T_macro']].copy()
        else:
        ...
        export_hourly_temperature_tables(micro, macro, merged, sites, OUTPUT_DIR)
