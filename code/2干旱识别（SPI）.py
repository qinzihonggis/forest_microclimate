"""
福建省 2025 年 SPI1 干旱等级标签脚本

脚本功能：
1. 读取 2025 年 SPI1 NetCDF 结果。
2. 按截图红框中的 SPI 范围生成干旱等级标签。
3. 输出标签 NetCDF、逐月统计 CSV、站点提取 CSV、逐月空间分布图 PNG、逐月占比堆叠柱状图 PNG。
...
# ============================================================
# 输入文件：第一步 SPI 计算得到的 2025 年逐月 SPI1 结果。
SPI_FILE = Path(r"E:\forest_microclimate\ForestMicroclimate\results\SPI_result\Fujian_SPI1_2025.nc")

# 输出目录：所有图、表、标签 NC 都写入该目录。
OUTPUT_DIR = Path(r"E:\forest_microclimate\ForestMicroclimate\results\SPI_mark")

# 输出 NC 文件名按要求使用英文小写，其他图表文件使用中文命名。
OUTPUT_NC = OUTPUT_DIR / "fujian_drought_label_2025.nc"
OUTPUT_CSV = OUTPUT_DIR / "福建省2025年SPI1干旱等级逐月统计表.csv"
OUTPUT_SITE_CSV = OUTPUT_DIR / "福建省2025年SPI1站点干旱等级与SPI值表.csv"
OUTPUT_LINE_CHART = OUTPUT_DIR / "福建省2025年SPI1干旱等级逐月占比堆叠柱状图.png"
...
# 1. 数据参数：变量名、年份、坐标名
# ============================================================
# SPI_VAR：NetCDF 中待标记的 SPI 变量名。当前文件中变量名为 SPI1。
SPI_VAR = "SPI1"
...
# ============================================================
# 4. 绘图参数：地图、图例、堆叠柱状图参数集中放置，便于后续调整
# ============================================================
MAP_CONFIG = {
...
LINE_CHART_CONFIG = {
    # figsize / dpi：堆叠柱状图尺寸和分辨率。
    "figsize": (10.5, 6.2),
    "dpi": 300,
    # bar_width：每个月柱子的宽度，越大柱子越宽。
    "bar_width": 0.68,
    # alpha：柱体透明度。
    "alpha": 0.95,
    # edgecolor / edge_linewidth：柱体边框颜色和线宽。
    "edgecolor": "#FFFFFF",
    "edge_linewidth": 0.6,
    # grid_alpha：背景网格透明度。
    "grid_alpha": 0.28,
    ...
    # segment_label_min_pct：堆叠柱段占比达到该阈值才标注数值，避免小柱段文字重叠。
    "segment_label_min_pct": 1.0,
    # segment_label_format：柱段数值标注格式，{value:.1f}% 表示保留 1 位小数。
    "segment_label_format": "{value:.1f}%",
    # segment_label_fontsize / segment_label_color：柱段标注字号和颜色。
    "segment_label_fontsize": 7,
    "segment_label_color": "#222222",
...
def build_site_monthly_table(spi_2025: xr.DataArray, ds_label: xr.Dataset, site_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, site in make_progress(
        ...
        for month_index, month_time in enumerate(month_times):
            ...
            row[f"{month_prefix}_SPI1"] = round(spi_value, 4) if np.isfinite(spi_value) else np.nan
...
def draw_monthly_maps(ds_label: xr.Dataset, site_df: pd.DataFrame) -> None:
    """逐月绘制 SPI1 干旱等级空间分布图，并叠加福建省行政边界和站点位置。"""
    ...
    for month_index in make_progress(
        ...
        month_text = month_time.strftime("%Y-%m")
        ...
        fig.tight_layout()
        fig_path = OUTPUT_DIR / f"福建省{month_time.strftime('%Y年%m月')}SPI1干旱等级空间分布图.png"
        fig.savefig(fig_path, dpi=MAP_CONFIG["dpi"], bbox_inches="tight")
        plt.close(fig)


def draw_monthly_line_chart(df_stat: pd.DataFrame) -> None:
    """绘制逐月干旱等级占比堆叠柱状图，柱体总高度即受旱网格占比。"""
    month_abbr = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    x_labels = month_abbr[: len(df_stat)]
    ...
    bottom_values = np.zeros(len(df_stat), dtype=float)
    ...
    for column_name, label, color in make_progress(
        ...
        y_values = df_stat[column_name].to_numpy(dtype=float)
        ax.bar(
            x_positions,
            y_values,
            bottom=bottom_values,
            width=LINE_CHART_CONFIG["bar_width"],
            label=label,
            color=color,
            alpha=LINE_CHART_CONFIG["alpha"],
            edgecolor=LINE_CHART_CONFIG["edgecolor"],
            linewidth=LINE_CHART_CONFIG["edge_linewidth"],
        )

        for x_value, segment_bottom, segment_height in zip(x_positions, bottom_values, y_values):
            if segment_height < LINE_CHART_CONFIG["segment_label_min_pct"]:
                continue

            ax.text(
                x_value,
                segment_bottom + segment_height / 2,
                LINE_CHART_CONFIG["segment_label_format"].format(value=segment_height),
                ha="center",
                va="center",
                fontsize=LINE_CHART_CONFIG["segment_label_fontsize"],
                color=LINE_CHART_CONFIG["segment_label_color"],
            )

        bottom_values += y_values

    ax.set_title(
        "Monthly SPI1 Drought Level Percentage in Fujian, 2025",
    ...
    ax.set_xticklabels(x_labels, rotation=LINE_CHART_CONFIG["tick_rotation"], ha="right")
...
def main() -> None:
    ...
    try:
        ...
        print(f"3. 逐月统计表：{OUTPUT_CSV}")
        print(f"4. 逐月空间分布图：{OUTPUT_DIR}\\福建省YYYY年MM月SPI1干旱等级空间分布图.png")
        print(f"5. 逐月占比堆叠柱状图：{OUTPUT_LINE_CHART}")
