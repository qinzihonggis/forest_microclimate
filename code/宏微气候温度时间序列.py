...
class Config:
    ...
    # hourly_aligned_min_utc_time：仅用于“逐小时温度对齐表.csv”的 UTC 起始时间筛选。
    # 导出该表前会删除 Time_UTC 早于该时刻的行；其他图件和表格不受该参数影响。
    hourly_aligned_min_utc_time: str = "2025-01-20 00:00:00"
...
def prepare_hourly_aligned_table(merged_hourly: pd.DataFrame, sites: pd.DataFrame) -> pd.DataFrame:
    ...
    output = output.rename(
        columns={
            ...
            "日期": "Date_Beijing",
            "ERA5_2m小时气温_摄氏度": "ERA5_T2m_C",
            "TOMST_15cm小时平均气温_摄氏度": "Observed_T15cm_C",
            "TOMST_小时内有效记录数": "Observed_Valid_15min_Count",
            "是否两者都有数据": "Has_Both_Data",
        }
    )
    columns = [
        "Site_ID",
        ...
        "ERA5_T2m_C",
        "Observed_T15cm_C",
        "Observed_Valid_15min_Count",
        "Has_Both_Data",
    ]
    output = output[columns]
    output["Time_UTC"] = pd.to_datetime(output["Time_UTC"], errors="coerce")
    output = output[output["Time_UTC"] >= pd.Timestamp(CONFIG.hourly_aligned_min_utc_time)].copy()
...
def save_outputs(
    ...
    with make_bar(
        ...
        era5_hourly_extract_table.to_csv(
            output_dir / CONFIG.era5_hourly_extract_table_name,
            index=False,
            encoding=CONFIG.output_csv_encoding,
        )
        bar.update(1)
        ...
        hourly_aligned_table.to_csv(
            output_dir / CONFIG.hourly_aligned_table_name,
            index=False,
            encoding=CONFIG.output_csv_encoding,
        )
        bar.update(1)
