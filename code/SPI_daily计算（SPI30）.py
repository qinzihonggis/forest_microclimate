# =============================================================
# 福建省 2025 年逐日 SPI1（SPI_30d）计算脚本
# =============================================================
# 方法说明：
#   本脚本使用 climate_indices 官方包计算 daily SPI。
#   daily SPI1 在这里定义为 30 天滑动累计降雨对应的 SPI，即 SPI_30d。
#
# 数据要求：
...
OUTPUT_DIR = Path(r"E:\forest_microclimate\ForestMicroclimate\results\daily_SPI_result")
...
# SPI1（日尺度）采用 30 天滑动累计降雨；按你的要求不计算 90 天或更长尺度。
SPI_SCALE_DAYS = 30
...
OUT_NC = OUTPUT_DIR / f"Fujian_daily_SPI30d_{TARGET_YEAR}.nc"
OUT_CSV = OUTPUT_DIR / f"逐日SPI30d全省统计_{TARGET_YEAR}.csv"
OUT_FIG = OUTPUT_DIR / f"福建省逐日平均SPI30d时间序列图_{TARGET_YEAR}.png"
OUT_QUANTILE_FIG = OUTPUT_DIR / f"SPI30d全省极值与分位数图_{TARGET_YEAR}.png"
OUT_DROUGHT_RATIO_FIG = OUTPUT_DIR / f"SPI30d干旱面积比例图_{TARGET_YEAR}.png"
OUT_METHOD_XLSX = OUTPUT_DIR / "Daily_SPI30d计算方法说明.xlsx"

# 真实站点坐标表：必须包含 Site_ID、Longitude、Latitude 三列。
SITE_CSV = Path(r"E:\forest_microclimate\ForestMicroclimate\Tensor_LatLong.csv")
OUT_SITE_SPI_XLSX = OUTPUT_DIR / f"各站点SPI30d逐日宽表_{TARGET_YEAR}.xlsx"
SITE_FIG_DIR = OUTPUT_DIR / f"各站点SPI30d时间序列图_{TARGET_YEAR}"
...
XTICK_ROTATION = 30
POSITIVE_SPI_LABEL = "SPI30d >= 0"
NEGATIVE_SPI_LABEL = "SPI30d < 0"
...
def calculate_spi30_from_prerolled(P_366: np.ndarray) -> np.ndarray:
    """
    对预先累计好的 30 天降雨逐网格调用 climate_indices.indices.spi() 计算 SPI_30d。
    ...
    failed_count = 0
    for grid_idx in progress_bar(range(n_grids), desc="计算逐网格SPI30d", unit="grid", colour="red"):
    ...
    stats_df = pd.DataFrame(
        {
            ...
            "全省平均SPI30d": np.nanmean(spi, axis=(1, 2)),
            "全省最小SPI30d": np.nanmin(spi, axis=(1, 2)),
            "全省最大SPI30d": np.nanmax(spi, axis=(1, 2)),
            "全省P5_SPI30d": spi_p5,
            "全省P50_SPI30d": spi_p50,
            "全省P95_SPI30d": spi_p95,
...
def save_method_workbook() -> None:
    ...
    workflow_rows = [
        ...
        ("7. 输出", "输出 2025 年逐日 SPI30d NC、统计表、图和站点宽表。", "同时满足栅格分析、省域概览和站点干旱事件识别。"),
    ]
    concept_rows = [
        ("为什么不是单日 SPI？", "单日降雨零值多、随机性强，直接标准化不稳定；30天累计更接近短期农业/生态干旱过程。"),
        ("为什么叫 SPI30d？", "因为每一天的 SPI 是基于该日及此前 29 个真实日累计降雨计算得到。"),
        ("为什么用全省平均 SPI30d 作主图？", "主图用于表达福建省整体干湿过程；平均值比单点更代表省域总体状态。"),
        ("为什么还输出分位数图？", "分位数图展示空间异质性：平均偏干时，是全省都干，还是局地特别干。"),
        ("为什么还输出干旱面积比例？", "面积比例回答干旱影响范围，即多少比例网格达到轻旱、中旱、重旱或特旱。"),
        ("为什么站点值叫邻近网格值？", "CHIRPS 是栅格数据，站点 SPI 是按站点坐标提取最近栅格的 SPI30d，不是实测站降雨直接计算。"),
    ]
    output_rows = [
        ("Fujian_daily_SPI30d_2025.nc", "2025 年福建省逐日 SPI30d 栅格结果。"),
        ("逐日SPI30d全省统计_2025.csv", "每日省平均、极值、分位数、干旱面积比例和有效网格数。"),
        ("福建省逐日平均SPI30d时间序列图_2025.png", "省域整体干湿变化主图。"),
        ("SPI30d全省极值与分位数图_2025.png", "空间分布范围和异质性辅助图。"),
        ("SPI30d干旱面积比例图_2025.png", "不同干旱等级影响范围图。"),
        ("各站点SPI30d逐日宽表_2025.xlsx", "站点编号为列、日期为行的逐日 SPI30d 宽表。"),
        ("各站点SPI30d时间序列图_2025", "逐站点邻近网格 SPI30d 时间序列图文件夹。"),
    ]
    qa_rows = [
        ...
        ("问：站点 SPI 是否等于站点实测 SPI？", "答：不是。这里是站点坐标对应的 CHIRPS 最近网格 SPI30d，用于与站点位置关联分析。"),
...
def save_site_spi_outputs(ds_out: xr.Dataset) -> None:
    """
    按真实站点坐标提取最近 CHIRPS 网格 SPI30d，并输出站点宽表和逐站点图。

    注意：
      CHIRPS 是栅格数据，站点坐标通常不完全落在网格中心。
      这里使用 method='nearest' 提取最近网格值，所以图和表代表“站点邻近网格 SPI30d”。
    """
    ...
    spi_da = ds_out["SPI_30d"]
    for _, row in progress_bar(sites.iterrows(), total=len(sites), desc="提取并绘制站点SPI30d", unit="site", colour="cyan"):
    ...
    with progress_bar(range(1), desc="保存站点SPI30d宽表", unit="file", colour="blue"):
        with pd.ExcelWriter(OUT_SITE_SPI_XLSX, engine="openpyxl") as writer:
            pd.DataFrame(site_spi).to_excel(writer, sheet_name="站点SPI30d宽表", index=False)
            pd.DataFrame(nearest_info).to_excel(writer, sheet_name="站点最近网格信息", index=False)
    print(f"站点SPI30d宽表已保存：{OUT_SITE_SPI_XLSX}")
    print(f"站点SPI30d时间序列图目录：{SITE_FIG_DIR}")


def make_single_site_plot(site_id: str, times: pd.DatetimeIndex, site_series: np.ndarray) -> None:
    """绘制单个站点邻近网格 SPI30d 时间序列图。"""
    ...
    ax.set_ylabel("SPI30d", fontsize=AXIS_LABEL_FONT_SIZE)
    ...
    fig.savefig(SITE_FIG_DIR / f"{site_id}_SPI30d时间序列.png", dpi=FIG_DPI, bbox_inches="tight")
    plt.close(fig)
...
def make_mean_spi_plot(stats_df: pd.DataFrame) -> None:
    """绘制正式主图：福建省平均 SPI30d 时间序列。"""
    plt.rcParams["font.sans-serif"] = FONT_FAMILY
    plt.rcParams["axes.unicode_minus"] = False

    times = pd.to_datetime(stats_df["日期"])
    province_mean = stats_df["全省平均SPI30d"].to_numpy()
    y_limits = get_spi_y_limits(province_mean, fixed_limits=MEAN_SPI_Y_LIMITS)

    with progress_bar(total=1, desc="绘制省平均SPI30d主图", unit="fig", colour="yellow") as pbar:
        ...
        add_drought_threshold_lines(ax)
        ax.set_title("福建省平均 SPI30d", fontsize=TITLE_FONT_SIZE)
        ax.set_ylabel("SPI30d", fontsize=AXIS_LABEL_FONT_SIZE)
        ax.set_xlabel("Date", fontsize=AXIS_LABEL_FONT_SIZE)
        ax.set_ylim(*y_limits)
        ax.grid(alpha=GRID_ALPHA)
    ...
    print(f"省平均SPI30d主图已保存：{OUT_FIG}")


def make_quantile_plot(stats_df: pd.DataFrame) -> None:
    """绘制全省 SPI30d 最小值、最大值和 P5/P50/P95 分位数图。"""
    times = pd.to_datetime(stats_df["日期"])
    spi_min = stats_df["全省最小SPI30d"].to_numpy()
    spi_max = stats_df["全省最大SPI30d"].to_numpy()
    spi_p5 = stats_df["全省P5_SPI30d"].to_numpy()
    spi_p50 = stats_df["全省P50_SPI30d"].to_numpy()
    spi_p95 = stats_df["全省P95_SPI30d"].to_numpy()
    ...
    with progress_bar(total=1, desc="绘制全省极值与分位数图", unit="fig", colour="yellow") as pbar:
        ...
        add_drought_threshold_lines(ax)
        ax.set_title("福建省 SPI30d 极值与分位数", fontsize=TITLE_FONT_SIZE)
        ax.set_ylabel("SPI30d", fontsize=AXIS_LABEL_FONT_SIZE)
        ax.set_xlabel("Date", fontsize=AXIS_LABEL_FONT_SIZE)
        ax.set_ylim(*y_limits)
        ax.grid(alpha=GRID_ALPHA)
...
def make_drought_ratio_plot(stats_df: pd.DataFrame) -> None:
    ...
    with progress_bar(total=1, desc="绘制干旱面积比例图", unit="fig", colour="yellow") as pbar:
        ...
        ax.set_title("福建省 SPI30d 干旱面积比例", fontsize=TITLE_FONT_SIZE)
...
def main() -> None:
    ...
    try:
        ...
        print("说明：P_366 中的值已经是 30 天累计降雨，后续 climate_indices 使用 scale=1。")

        print_step("步骤4：调用climate_indices对30天累计降雨做Gamma标准化")
        spi_flat = calculate_spi30_from_prerolled(P_366)
        ...
        ds_out = xr.Dataset(
            data_vars={
                "SPI_30d": (
                    ...
                    spi_2025,
                    {
                        "long_name": "Daily SPI1 based on 30-day accumulated precipitation",
                        "units": "dimensionless",
                        "scale_days": str(SPI_SCALE_DAYS),
                        "calibration": f"{CALIB_START_YEAR}-{CALIB_END_YEAR}",
                        "method": "30-day precipitation pre-rolled on real calendar; climate_indices.indices.spi scale=1; Gamma distribution; daily periodicity",
                    },
                )
            },
            coords={
            ...
            attrs={
                "title": f"Fujian daily SPI_30d for {TARGET_YEAR}",
                ...
                "rolling_note": "30-day rolling sum is computed before inserting non-leap Feb-29 placeholders",
            },
        )
