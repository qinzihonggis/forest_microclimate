...
def get_balanced_nondrought_period(site_daily_spi, d_start, d_end, gap_days):
    ...
    target_days = len(pd.date_range(d_start, d_end, freq='D'))
...
def compute_paired_betas(merged, drought_df, daily_spi_df, sites_df, gap_days=CONTROL_GAP_DAYS):
    ...
    for _, ev in tqdm(
        ...
        # ════════════════════════════════════════════════
        # 4d. 综合非干旱期β（两期配对：Non-drought vs Drought）
        # 目标：构建“总天数 = 干旱期天数”的综合非干旱期
        #      前后两侧尽量平衡，一侧不足时由另一侧补足。
        #      每一侧都只允许使用与目标事件相邻的一段连续非干旱日；
        #      如果继续向外遇到干旱日或缺失SPI，就停止该侧，不跨越该断点。
        # ════════════════════════════════════════════════
        nondrought_info = get_balanced_nondrought_period(site_daily_spi, d_start, d_end, gap_days)
        nondrought_days = nondrought_info['used_days']
        if nondrought_days:
            nondrought_hours = site_data[
                site_data.index.normalize().isin(nondrought_days)
            ]
...
def analyze_and_plot(beta_table, control_quality, site_coverage, output_dir):
    ...
    # 参考：De Frenne et al. (2021) Nature Ecology & Evolution
    site_summary = (
        beta_table.groupby('site_id')[['beta_drought', 'beta_pre', 'beta_post', 'beta_nondrought']]
        .median()
        .reset_index()
    )
    ...
    report_nondrought = do_wilcoxon(
        ...
        "干旱期 vs 综合非干旱期（Balanced non-drought control）"
    )
    ...
    if not plot_long_two.empty:
        ...
        ax.set_title(
            "Overall Comparison of Climate Buffering Index\n"
            "Non-drought vs Drought (site-level summary)",
            fontsize=TITLE_FONTSIZE, pad=12
        )
        plt.tight_layout()
    ...
    # ── 5h. Site-level plots: paired line plot and event bar plot（3-period） ──
    ...
    # ── 5i. Site-level plots: Non-drought vs Drought（2-period） ──
    site_event_nondrought_long = build_site_event_nondrought_long(beta_table)
    for site_id, site_plot_df in site_event_nondrought_long.groupby('site_id'):
        ...
        ax.set_xticks(event_order)
        ax.set_xticklabels(
            [site_plot_df[site_plot_df['event_order'] == i]['event_label'].iloc[0] for i in event_order],
            fontsize=9
        )
        ax.set_ylabel("Climate Buffering Index (β)", fontsize=AXIS_LABEL_FONTSIZE)
        ax.set_xlabel("Extreme Drought Events (chronological order)", fontsize=AXIS_LABEL_FONTSIZE)
        ax.set_title(f"Site {site_id}: Drought vs Non-drought β Bars", fontsize=TITLE_FONTSIZE, pad=10)
        ax.legend(title="Period", fontsize=LEGEND_FONTSIZE, title_fontsize=LEGEND_FONTSIZE, loc='best')
        plt.tight_layout()
        bar_path_two = os.path.join(site_plot_dir, f"站点_{sanitize_filename(site_id)}_干旱与非干旱柱状图.png")
        plt.savefig(bar_path_two, dpi=FIG_DPI, bbox_inches='tight')
        plt.close()

    # ── 5j. Site-level summary paired plots: all events collapsed into two groups ──
    # 图形目的：
