...
整体目标
1. 从 daily SPI30d NetCDF 中读取 2025 年逐日 SPI 栅格数据。
...
# 当前脚本要求该文件至少包含：
# 1. 变量 SPI_VAR，当前为 "SPI_30d"；
# 2. 时间维度 TIME_DIM，当前为 "time"；
# 3. 纬度和经度坐标 LAT_NAME / LON_NAME，当前为 "lat" / "lon"；
# 4. 2025-01-01 到 2025-12-31 的逐日数据。
SPI_FILE = Path(
    r"E:\forest_microclimate\ForestMicroclimate\results\daily_SPI_result\Fujian_daily_SPI30d_2025.nc"
)

# OUTPUT_DIR：
# 所有正式输出文件写入该目录。
# 脚本会自动创建该目录；不会主动删除该目录下已有正式结果。
OUTPUT_DIR = Path(r"E:\forest_microclimate\ForestMicroclimate\results\daily_SPI_features")
...
OUTPUT_GRID_EVENT_NC = OUTPUT_DIR / "fujian_daily_spi_drought_events_2025.nc"

# OUTPUT_SITE_EVENT_CSV：
# 站点事件长表。
# 一行代表一个站点的一次有效干旱事件，同一站点全年多次事件会占多行。
OUTPUT_SITE_EVENT_CSV = OUTPUT_DIR / "福建省观测站2025年daily_SPI干旱事件长表.csv"

# OUTPUT_SITE_ANNUAL_CSV：
# 站点年度统计表。
# 一行代表一个站点，汇总该站点全年 SPI、干旱日数、事件次数和最严重事件等指标。
OUTPUT_SITE_ANNUAL_CSV = OUTPUT_DIR / "福建省观测站2025年daily_SPI干旱年度统计表.csv"

# OUTPUT_SITE_DAILY_CSV：
# 站点逐日 SPI 序列表。
# 一行代表一个站点的一天，便于后续按任意时段重新筛选或核查事件。
OUTPUT_SITE_DAILY_CSV = OUTPUT_DIR / "福建省观测站2025年daily_SPI逐日序列表.csv"

# OUTPUT_SCATTER_PNG：
# 站点事件严重度-持续时间散点图。
# 一点代表一个站点的一次有效干旱事件，横轴为 Duration_Days，纵轴为 Severity。
OUTPUT_SCATTER_PNG = OUTPUT_DIR / "福建省观测站2025年daily_SPI严重度-持续时间散点图.png"

# OUTPUT_SCATTER_NOTE_PNG：
# 站点严重度-持续时间散点图说明。
# 单独输出为 PNG，记录点、颜色、形状、参考线和事件定义的解释。
OUTPUT_SCATTER_NOTE_PNG = OUTPUT_DIR / "福建省观测站2025年daily_SPI严重度-持续时间散点图说明.png"

# 敏感性分析输出：
# 目的不是替代主分析，而是评估站点取值方法选择对结果的影响。
OUTPUT_SENSITIVITY_DAILY_CSV = OUTPUT_DIR / "福建省观测站2025年daily_SPI取值方法逐日对比表.csv"
OUTPUT_SENSITIVITY_EVENT_CSV = OUTPUT_DIR / "福建省观测站2025年daily_SPI敏感性分析事件长表.csv"
OUTPUT_SENSITIVITY_ANNUAL_CSV = OUTPUT_DIR / "福建省观测站2025年daily_SPI敏感性分析年度统计表.csv"
OUTPUT_SENSITIVITY_DELTA_CSV = OUTPUT_DIR / "福建省观测站2025年daily_SPI敏感性分析差异表.csv"
OUTPUT_SENSITIVITY_SUMMARY_CSV = OUTPUT_DIR / "福建省观测站2025年daily_SPI敏感性分析总体统计表.csv"
OUTPUT_SENSITIVITY_FIG_PNG = OUTPUT_DIR / "福建省观测站2025年daily_SPI敏感性分析图.png"
...
RUN_CACHE_DIR = OUTPUT_DIR / "_本次运行缓存_提取daily_SPI干旱事件"
...
# 当前 daily SPI 计算脚本输出的是 SPI_30d，表示 30 日累计降水对应的逐日 SPI。
SPI_VAR = "SPI_30d"
                ...
                daily_rows.append(
                    {
                        **common_fields,
                        ...
                        "Daily_SPI_30d": round(float(spi_val), 4)
...
def create_severity_duration_caption_png(output_file: Path) -> None:
    ...
    note_text = (
        ...
        f"at least {MIN_DURATION_DAYS} consecutive days with daily SPI30d < {DROUGHT_THRESHOLD}.\n"
